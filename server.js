    ...
    app.get('/api/steal-items', async (req, res) => {
      try {
        const stealItems = await Steal.find().populate('itemId').exec();
        res.status(200).json(stealItems);
      } catch (error) {
        console.error('Error fetching steal items:', error);
        res.status(500).json({ error: 'Failed to fetch steal items.' });
      }
    });
